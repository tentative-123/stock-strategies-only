...
def _stock_card(stock: dict, kind: str) -> str:
    ...
    chip_text = "、".join(components.get("chip_signals", [])[:2]) or "法人籌碼中性"
    ...
    return f"""
      <article class="stock-card {kind}">
        ...
        <div class="analysis chip-reading"><b>籌碼解讀</b><span>{_escape(chip_text)}｜回測 {chip_wr_text}（{components.get('chip_backtest_samples', 0)}次）</span></div>
